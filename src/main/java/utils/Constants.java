package utils;

public class Constants {

    public final static int PARSER_NUMBER = 32;
    public final static int FETCHER_NUMBER = 128;
    public final static int LRU_TIME_LIMIT = 30;
    public final static int FETCH_TIMEOUT = 1000;
    public final static String HBASE_TABLE_NAME = "a6";
    public final static String HBASE_FAMILY_NAME = "f1";
    public final static String URL_TOPIC = "newUrl5";
    public final static int STATISTIC_REFRESH_TIME =  10000;

    public final static int FETCHED_DATA_QUEUE_SIZE =  1000;
    public final static int URL_QUEUE_SIZE = 2000;

    public final static int KAFKA_SLEEP_TIME = 200;

}
